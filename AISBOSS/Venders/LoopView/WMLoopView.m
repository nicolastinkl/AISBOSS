...
#define kBannerDefaultImage  [UIImage imageNamed:@"default-chart02"]
...
- (void)addPageControl{
    ...
    bgView.backgroundColor = [UIColor clearColor];//[UIColor colorWithRed:0.3 green:0.3 blue:0.3 alpha:0.2];
...
- (NSMutableArray *)currentTitles{
    ...
    NSInteger count = self.imageTitles.count;
    int i = (int)(_currentPage + count - 1)%count;
    [_currentTitles addObject:self.imageTitles[i]];
    [_currentTitles addObject:self.imageTitles[_currentPage]];
...
- (void)addScrollView{
    ...
    scrollView.contentSize = CGSizeMake(self.currentImages.count*width, height);
...
- (void)refreshImages{
    ...
    for (int i = 0; i < subViews.count; i++) {
        ...
        // MARK: 网络下载图片
        NSURL *url = [NSURL URLWithString:self.currentImages[i]];
        [imageView sd_setImageWithURL:url placeholderImage:kBannerDefaultImage];
